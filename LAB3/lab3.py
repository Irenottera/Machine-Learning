...
def mlParams(X, labels, W=None):
    ...
    for jdx,c in enumerate(classes):
        ...
        xlcc = X[idx,:]*W[idx]
        n = len(xlcc)
        mu[jdx] = np.sum(xlcc, axis=0)/ np.sum(W[idx])  #I sum the columns
        #Diag = np.square ( np.sum ( (xlc-mu[jdx]) , axis=0 ) ) #at column m of xlc I subtract the m-th component of mu, then I sum the columns and square them
        diff = xlc - mu[jdx] # Matrix  C x d with diffs between x - µ
        diff = W[idx]*np.square(diff)
...
def classifyBayes(X, prior, mu, sigma):
    ...
    # Now I compute a matrix contaning the posterior estimate for every point wrt every class
    for jdx in range(Nclasses):
        central = np.zeros(Npts)
        muk = mu[jdx]
        sigmak = sigma[jdx]
        lnsigmak = -0.5*np.log(np.linalg.det(sigmak))
        lnprior = np.log(prior)
        for i in range(Npts):
            diff = X[i]-muk
            central[i] = -0.5*np.inner(diff/np.diag(sigma[jdx]), diff)
        #print('LNSIGMAK')
        #print(lnsigmak)
        #print('CENTRAL')
        #print(central)
        #print('LNPRIOR')
        #print(lnprior)
        logProb[jdx] = lnsigmak + central + lnprior[jdx]


    # now for each point I chose the class that give the maximum posterior value
...
def classifyBoost(X, classifiers, alphas, Nclasses):
    ...
    if Ncomps == 1:
        ...
        for i in range(Ncomps):
            classification_i = classifiers[i].classify(X)
            for j in range(Npts):
                votes[j][classification_i[j]] += alphas[i]
